...
public abstract class AbstractPaxmlEntityFactory implements IEntityFactory {
    ...
    private static final class ParserContext implements IParserContext {
        ...
        private ParserContext(final LinkedList<IParserContext> stack, final OMElement element,
                final PaxmlResource resource, final ResourceLocator locator, final IEntity entity,
     ...
     * @return the new parser context which is not null
     */
    public static IParserContext createParserContext(OMElement ele, ITag parentTag, PaxmlResource resource,
            ResourceLocator locator, IEntity entity, LinkedList<IParserContext> stack) {
    ...
    private static Class<? extends ITag> getTagImplClass(OMElement ele, ParserContext context) {
        ...
        } else if (Namespaces.DATA.equals(ns)) {
            clazz = ConstTag.class;
        ...
        if (clazz == null) {
            throw new PaxmlRuntimeException("Cannot recognize tag name '" + ele.getLocalName() + "' under namespace: "
                    + ns);
        }
        return clazz;
    }
    ...
    private ITagFactory doCreateTagFactory(Class<?> clazz) {
        ...
        if (a == null) {
            throw new PaxmlParseException("Tag class '" + clazz.getName() + "' has no @" + Tag.class.getSimpleName()
                    + " annotation on itself nor its super classes");
        }
