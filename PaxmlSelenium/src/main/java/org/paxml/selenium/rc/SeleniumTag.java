...
public abstract class SeleniumTag extends BeanTag {
	                    ...
	                    try {
	                        ...
	                        if (keep) {
	                            if (log.isWarnEnabled()) {
	                                log.warn("The selenium session is not closed in the end,"
	                                        + " because the last <url> tag has @keepSessionOnError='true'!");
	                            }
	                        } else {
     ...
     * @param selenium the target selenium. null means to get the 1st created selenium in the context
		...
		if (selenium == null) {
			...
			if (selenium == null) {
				throw new PaxmlRuntimeException(
						"There is no active session found to switch to!");
			}
		} else if (selenium.isTerminated()) {
			throw new PaxmlRuntimeException(
					"The target session is already closed, you cannot switch to it any more!");
		}
		XSelenium old = getSelenium(context);
		context.setInternalObject(PrivateKeys.SELENIUM, selenium, true);
		if (log.isInfoEnabled()) {
			log.info("Selenium session switched from " + old + " to: "
					+ selenium);
		}
		return selenium;
	}
     ...
     * @param context
     *            the context
     * @param forceNew force to create a new selenium session     
    ...
    public static XSelenium getSelenium(Context context, boolean forceNew, String browser) {
        ...
        if (selenium == null) {
            ...
            if (StringUtils.isBlank(browser)) {
                noPropertyGivenException(SELENIUM_BROWSER_IDENTIFIER);
            }
            ...
            if (path != null) {
                if (selenium.isSnapshotSupported()) {
                    if (log.isInfoEnabled()) {
                        log.info("Selenium snapshots will be taken upon any test errors, and save to this folder: "
                                + path);
                    }
                } else {
                    if (log.isInfoEnabled()) {
                        log.info("The current browser does not support taking snapshots: "
                                + selenium.getBrowserStartCommand());
                    }
                }
            } else {
                log.info("Selenium snapshots will NOT be taken upon test errors, because the snapshot storage folder is not given n this property: "
                        + SELENIUM_SNAPSHOT_FOLDER);
            }
        }
