...
import org.paxml.control.AbstractControlTagFactory;
...
public abstract class AbstractTagFactory<T extends ITag> implements ITagFactory<T> {
		...
		traverseAttributes(ele, new IAttributeVisitor() {
			...
			public void visit(OMElement ele, String attrName, String attrValue) {
				if (!set.contains(attrName)) {
					throw new PaxmlRuntimeException("No attribute '" + attrName + "' should exist in tag: " + ele.getLocalName());
				}
			}
		});
	}
	...
	public static Condition parseConditions(OMElement ele, String ifAttr, String unlessAttr) {
		...
		if (!StringUtils.isBlank(unlessCond)) {
			if (cond != null) {
				throw new PaxmlRuntimeException("Cannot have both conditional attributes specified: " + ifAttr + "," + unlessAttr);
			}
			cond = new Condition();
	...
	public static boolean isConstTag(OMElement ele, IParserContext context) {
		return AbstractPaxmlEntityFactory.isCreatedByTagFactory(ele, context, ConstTagFactory.class);
	}
	...
	public static boolean isControlTag(OMElement ele, IParserContext context) {
		return AbstractPaxmlEntityFactory.isCreatedByTagFactory(ele, context, AbstractControlTagFactory.class);
	}
	...
	public static boolean isInvokerTag(OMElement ele, IParserContext context) {
		return AbstractPaxmlEntityFactory.isCreatedByTagFactory(ele, context, InvokerTagFactory.class);
	}
	...
	/**
	 * Create an instance from the element.
	 * @param ele the xml element
	 * @param context the parser context
	 * @return the instance, never null
	 */
	protected abstract T createTagInstance(OMElement ele, IParserContext context);
	
	@Override
    public TagCreationResult<T> create(OMElement ele, IParserContext context) {
		T tag = createTagInstance(ele, context);
